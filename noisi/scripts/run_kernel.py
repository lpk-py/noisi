...
def paths_input(cp,source_conf,step):
    ...
    # careful: if the spectrum is updated, the spectrum should no longer be
    # part of the kernel. (Now only geographic weights are updated)
...
def g1g2_kern_simple(wf1,wf2,kernel,adjt,
    src,source_conf):
            ...
            for i in range(wf1.stats['ntraces']):
               ...
               # c = np.multiply(g1g2_tr,S)
                ...
                for j in range(n_bases):
                    c = np.multiply(g1g2_tr,spectral_basis[j,:])
                    ...
                    # A Riemann sum 
                    kern[i,j] = np.dot(corr_temp,f.data) * f.stats.delta
...
def run_kernel(source_configfile,step):
    ...
    p = rem_fin_prs(p,source_config,step,kernelrun=True)
    ...
    for cp in p_p:
        ...
        g1g2_kern_simple(wf1,wf2,kernel,adjt,src,source_config)
